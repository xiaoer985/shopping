  ...
  render() {
    ...
    return (
      <div style={{ height: 700 }}>
        ...
        <div style={{ height: '25%' }}>
          <h3 style={{ textAlign: 'center' }}>{`当前合计金额为: ${amount.toFixed(2)}美刀`}</h3>
          <Button onClick={() => this.settlement()} disabled={amount.toFixed(2) <= 0.00} size="large" block>{key ? <div style={{ color: 'red' }}>正在结算中...</div> : <div>点击结算</div>}</Button>
